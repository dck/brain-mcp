...
#[test]
fn config_flag_reaches_spawned_server() {
    let tmp = tempfile::tempdir().unwrap();
...
"#,
            ...
            index = tmp.path().join("index.db").display(),
        ),
    )
    .unwrap();
    ...
    send(
        &mut stdin,
        r#"{"jsonrpc":"2.0","id":2,"method":"tools/call","params":{"name":"memory_list","arguments":{}}}"#,
    );
    let line = line_rx
        .recv_timeout(Duration::from_secs(12))
        .expect("tools/call response within 12s");
    ...
    assert_eq!(resp["result"]["isError"], true);
    let text = resp["result"]["content"][0]["text"].as_str().unwrap();
    assert!(text.contains("unknown embedding provider: nonexistent"));
